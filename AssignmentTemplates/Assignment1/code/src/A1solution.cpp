...
void A1solution::run(const std::string& filename) {
    ...
    std::vector<unsigned int> indices(M * 3);
    for (int i = 0; i < M * 3; i++) {
        file >> indices[i];
    }
    file.close();
    ...
    int shaderProgram = vertexColorProgram;
    ...
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, &modelViewMatrix[0][0]);
